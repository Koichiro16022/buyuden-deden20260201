...
if st.session_state.step == 1:
    ...
    if st.button("オチを出す", use_container_width=True, type="primary"):
        with st.spinner("思考中..."):
            p = f"{kw}の{target}向けオチを20案。ひらがな、4/4/5、スラッシュ区切りで。"
            try:
...
# --- STEP 2: オチ選択 ---
    ...
    if st.session_state.ochi_list:
        ...
        st.session_state.final_ochi = st.text_input("オチを修正", value=sel_ochi)
        
        col1, col2 = st.columns(2)
        with col1:
            if st.button("振りを出す", use_container_width=True, type="primary"):
                with st.spinner("思考中..."):
                    fp = f"オチ「{st.session_state.final_ochi}」への強気な振りを20案。ひらがな、4/4/5で。"
                    try:
